...
def load_data():
    ...
    # Reset matches & counts for new day but retain player names
...
def main():
    ...
    st.markdown(
        ...
        div[data-testid="stHorizontalBlock"] div[role="listitem"] { text-align: center !important; }
        .match-box {
    ...
    # ---------------- MATCH HISTORY ----------------
    st.markdown("<div class='section-title'>🎯 Match History (Today)</div>", unsafe_allow_html=True)
    ...
    if data["matches"]:
        for match in reversed(data["matches"]):  # latest first
            st.markdown(
                ...
                <div class='match-box'>
                    🕒 {match['time']} &nbsp;&nbsp;
                    👥 <b>{match['p1']} & {match['p2']}</b> vs <b>{match['p3']} & {match['p4']}</b>
                    <br>🗒️ Remark: {match['remark']}
                </div>
                """,
                unsafe_allow_html=True,
            )
    ...
    # ---------------- ADD MATCH ----------------
    st.markdown("<div class='section-title'>➕ Add New Match</div>", unsafe_allow_html=True)
    ...
    # ---------------- PLAYER STATISTICS ----------------
    st.markdown("<div class='section-title'>📊 Player Statistics</div>", unsafe_allow_html=True)
    if data["player_stats"]:
        stats_html = "<table style='width:100%; margin-top:10px; text-align:center;'><tr><th>Player</th><th>Matches Played</th></tr>"
    ...
    # ---------------- TOP ACTIVE PLAYERS ----------------
    st.markdown("<div class='section-title'>🔥 Top Active Players Today</div>", unsafe_allow_html=True)
    ...
    st.markdown("<div class='section-title'>🕒 Matches by Time Range</div>", unsafe_allow_html=True)
    if data["matches"]:
        ranges = {}
        for m in data["matches"]:
            r = m["range"]
            ranges[r] = ranges.get(r, 0) + 1
        html = "<table style='width:100%; text-align:center;'><tr><th>Time Range</th><th>Matches</th></tr>"
    ...
    # ---------------- MANAGE PLAYERS ----------------
    st.markdown("<div class='section-title'>⚙️ Manage Players</div>", unsafe_allow_html=True)
    st.write("Add or remove registered players below.")
